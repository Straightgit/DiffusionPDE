...
def get_pressure_loss(a, u, a_GT, u_GT, a_mask, u_mask, device=torch.device('cuda')):
...
def get_pressure_loss_observ(a, u, a_GT, u_GT, a_mask, u_mask, device=torch.device('cuda')):
    """Return the loss of the Darcy Flow equation and the observation loss."""
    observation_loss_a = (a - a_GT).squeeze()
    observation_loss_a = observation_loss_a * a_mask
    observation_loss_u = (u - u_GT).squeeze()
    observation_loss_u = observation_loss_u * u_mask
    # plt.imshow(u.detach().cpu().numpy().squeeze())
    # plt.colorbar()
    # plt.show()
    # plt.imshow(u_GT.detach().cpu().numpy().squeeze())
    # plt.colorbar()
    # plt.show()
    # u = u.view(1, 1, 128, 128)
    # u_GT = u_GT.view(1, 1, 128, 128)
...
def generate_pressure(config):
    ...
    for i, (sigma_t_cur, sigma_t_next) in tqdm.tqdm(list(enumerate(zip(sigma_t_steps[:-1], sigma_t_steps[1:]))), unit='step'): # 0, ..., N-1
        ...
        # Compute the loss
        observation_loss_a, observation_loss_u = get_pressure_loss_observ(a_N, u_N, a_GT, u_GT, known_index_a, known_index_u, device=device)
